...
def Pretext(
    ...
    for epoch in range(Epoch):
        ...
        for index, (aug1, aug2, neg) in enumerate(
            ...
            pos1_features = k_encoder(aug2, proj='top')  # (B, 128)
            neg1_features = k_encoder(neg, proj='top')  # (B, 128)
            
            anc2_features = q_encoder(aug2, proj='top') #(B, 128)
            pos2_features = k_encoder(aug1, proj='top')  # (B, 128)
            neg2_features = k_encoder(neg, proj='top')  # (B, 128)
           
            # backprop
            loss1 = criterion(anc1_features, pos1_features, neg1_features)
            loss2 = criterion(anc2_features, pos2_features, neg2_features)
            loss = (loss1 + loss2) / 2
